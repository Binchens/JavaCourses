    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (int i = 0; i<3; i++) {
            ...
            for (int j = 0; j<3; j++) {
                ...
                if(grid[i][j] == '-') oneIsEmpty = true;
            }
            if (col.contentEquals("xxx") || row.contentEquals("xxx")) result = "X wins";
            if (col.contentEquals("ooo") || row.contentEquals("ooo")) result = "O wins";
        }
        if (ltr.contentEquals("xxx") || rtl.contentEquals("xxx")) result = "X wins";
        if (ltr.contentEquals("ooo") || rtl.contentEquals("ooo")) result = "O wins";

        if(result == "None" && !oneIsEmpty) result = "Tie";
